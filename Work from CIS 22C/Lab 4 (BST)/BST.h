...
public:
	...
	void writeToFile2(Node<T>* root, ostream &out);
};

template<class T>
void BST<T>::writeToFile(ostream &out)
{
	if (root != NULL)
	{
		writeToFile2(root->left, out);
		out << root->data << "\n";
		writeToFile2(root->right, out);
	}
}

template<class T>
void BST<T>::writeToFile2(Node<T>* root, ostream &out)
{
	if (root != NULL)
	{
		writeToFile2(root->left, out);
